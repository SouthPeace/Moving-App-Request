...
class OrderTrackingPage4 extends StatefulWidget {
  const OrderTrackingPage4({Key? key}) : super(key: key);

  @override
  _OrderTrackingPageState4 createState() => _OrderTrackingPageState4();
// State<_OrderTrackingPage> createState() => _OrderTrackingPageState();
}

class _OrderTrackingPageState4 extends State<OrderTrackingPage4> {
  ...
  void getCurrentLocation() async {
    ...
    // set on the current location tracking
    if( switchoncurrentlocation == null ){
...
print(" GET CURRENT LOCATION 22 ");
  ...
  //=================
  // get poly lines 2
  // get walking polypoints
  void getpolylines2() async{

    polylineCoordinates.clear();

    PolylinePoints polylinePoints = PolylinePoints();

    GoogleMapPolyline googleMapPolyline = GoogleMapPolyline(apiKey:  google_api_key);

    List<LatLng>? _coordinates = await googleMapPolyline.getCoordinatesWithLocation(
        origin: LatLng(sourceLocation_lat,sourceLocation_lng),
        destination: LatLng(destination_lat,destination_lng),
        mode:  RouteMode.walking);

    print("here for new polyline");
    ...
    //   polylineId: PolylineId("there there33"),
    //   visible: true,
    ...
    // print('all string in list for points');
    // poly_points.forEach((element) {
    //   print(element);
    // });

    if(ending_ == null){

      print("number of polyline points:${polylineCoordinates.length}");
      for(var i = 0; i < polylineCoordinates.length-1; i++){
...
var distance_counter = 0;
    ...
    if( ending_ != null ) {
     //print("create string things");
     //print( poly_points.join('%7C') );
     // var result_string = create_poly_string.toString().substring(0,create_poly_string.toString().length-3);
      //print(result_string);
     //print('length of distances list:');
     //print( _distances.length );
      getgradiant(poly_points, points_counter, _distances);

    }

    //adding ellements to list
    int countingpoly = 0;

    // get the gradient and map onto polylines
  ...
  // get the elevaation between two points
   getgradiant(List<String> allpoints, int sample_count, List<double> _distances2) async{
    ...
    if (response.statusCode == 200) {
      ...
      double tt_slope = total_slope / sample_count;
